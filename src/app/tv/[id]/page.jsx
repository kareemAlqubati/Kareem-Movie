  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-gradient-to-b from-black via-gray-900 to-gray-800">
      <div
        ...
        style={{ backgroundImage: `url(https://image.tmdb.org/t/p/original/${movie.backdrop_path})`, filter: 'blur(12px)',
        }}
      />
      <div className="bg-black bg-opacity-80 p-6 rounded-lg shadow-2xl max-w-4xl w-full">
        <div className="flex flex-col md:flex-row md:space-x-8 items-start">
          <img  className="w-full md:w-1/3 rounded-lg shadow-md"  src={`https://image.tmdb.org/t/p/w500/${movie.poster_path}`}  alt={movie.title}  />
          ...
          {actors.slice(0, visibleActors).map((actor) => (
            <div key={actor.id} className="text-center">
              <Link href={`/actors/${actor.id}`}>
                <img  className="w-24 h-36 object-cover rounded-lg mx-auto shadow-md transition-transform transform hover:scale-105"
